        ...
        {stats.map((stat) => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <p className={`text-[10px] font-semibold tracking-widest uppercase ${stat.bg === '#E8604C' ? 'text-white/70' : 'text-[#94a3b8]'
                }`}>
                {stat.label}
              </p>
              <stat.icon className={`w-5 h-5 ${stat.bg === '#E8604C' ? 'text-white/60' : 'text-[#94a3b8]'}`} />
            </div>
            <p className={`text-3xl font-bold font-heading ${stat.bg === '#E8604C' ? 'text-white' : 'text-[#0b1c30]'
              }`}>
              {stat.value}
            </p>
            {stat.change && (
              <span className={`inline-flex items-center gap-1 mt-2 px-2 py-0.5 rounded-full text-xs font-medium ${stat.bg === '#E8604C' ? 'bg-white/20 text-white' : 'bg-[#ecfdf5] text-[#059669]'
                }`}>
          ...
          <div className="space-y-4">
            {logs.map((log, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${log.severity === 'error' ? 'bg-[#E8604C]' : log.severity === 'info' ? 'bg-[#001b26]' : 'bg-[#94a3b8]'
                  }`} />
